...
static HI_VOID *SAMPLE_VGS_Get_Frame_Thread(HI_VOID *arg)
{
    ...
    *************************************************/
    ...
    print_VI_CHN_ATTR_S(&stChnAttr);

//#ifndef AISA_EXTENSION
...
//#endif
	...
	printf("getting frame\n");
        ...
        *************************************************/
        ...
        if(HI_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("HI_MPI_VI_GetChnFrame failed! %s\n", hi_errstr(s32Ret));
	...
	printf("got frame\n");
        ...
        *************************************************/
        ...
        stVgsAddCover.enCoverType = COVER_RECT;  //Прямоугольная COVER
       ...
       /*****
       *Ниже приведен очень важный фрагмент кода.
       * HI_MPI_VGS_AddDrawLineTask,
       * HI_MPI_VGS_AddCoverTask,
       * HI_MPI_VGS_AddOsdTask
       * Входные и выходные изображения должны использовать один и тот же буфер.
       */
...
HI_S32 SAMPLE_VGS_SDR10_COMPRESS(VO_INTF_TYPE_E enVoIntfType)
{
    ...
    *************************************************/
    ...
    stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.enMastPipeMode = VI_ONLINE_VPSS_ONLINE; //VI_OFFLINE_VPSS_OFFLINE;
    ...
    stViConfig.astViInfo[s32WorkSnsId].stChnInfo.enCompressMode  = COMPRESS_MODE_NONE; //COMPRESS_MODE_SEG;
    ...
    *************************************************/
    ...
    printf("COMMON_GetPicBufferSize: %lu\n", u32BlkSize);
